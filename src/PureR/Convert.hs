...
instance Semigroup ModuleInfo where
  ModuleInfo fa ia <> ModuleInfo fb ib = ModuleInfo (fa || fb) (ia <> ib)

instance Monoid ModuleInfo where
  mempty = ModuleInfo False mempty
...
convert (Module spn _comments name path imports exports reexports foreign' decls)
  = flip runState mempty $ flip runReaderT (path, name, spn) $ module'
    name
    imports
    exports
    reexports
...
module' thisModule imports exports reexports foreign' decls = do
  let importBinding =
        let attrs =
              [ ( N.moduleKey mdl
                , N.source ("'../" <> P.runModuleName mdl <> "/default.R'")
                )
              | (_, mdl) <- imports
              , mdl /= thisModule
      ...
      expts    = (\x -> (N.identKey x, N.var $ N.mkVar x)) <$> exports
      reexpts  = uncurry inheritFrom <$> M.toList reexports
  ffiFileBinding <- if null foreign'
    then pure []
    else [("foreign", N.source ("'./foreign.R'"))]
      <$ tell mempty { usesFFI = True }
  binds <- bindings decls
  pure $ N.let' (importBinding : ffiFileBinding <> ffiBinds <> binds)
                (N.assoc $ expts <> concat reexpts)
 where
  inheritFrom :: P.ModuleName -> [Ident] -> [(N.Key, N.Expr)]
  inheritFrom m exps =
    (\e ->
        ( N.identKey e
  ...
  foreignBinding ffiIdent =
    (N.mkVar ffiIdent, N.sel (N.var "foreign") (N.identKey ffiIdent))
...
expr (Let ann binds body) =
  localAnn ann $ liftA2 N.let' (bindings binds) (expr body)
expr (ObjectUpdate ann a b) = localAnn ann $ liftA2 N.update (expr a) (attrs b)
expr (Constructor _ _ (P.ProperName dataName) fields) =
  pure $ N.constructor dataName (N.mkVar <$> fields)
expr (Case ann exprs cases) = localAnn ann $ do
  exprs'       <- traverse expr exprs
  cases'       <- traverse (alternative exprs') cases
  (fp, _, spn) <- ask
  let patternCases = zip (N.numberedVars "pattern__") cases'
      patternFail =
        ( "patternFail__"
        , (N.string $ T.concat
...
alternative scrutinees = go
 where
  go (CaseAlternative binders body) = do
    ...
    body'                         <- unguard body (N.var "fail__")
    pure $ N.lam "fail__" $ case patternChecks of
      [] -> N.let' patternBinds body'
      _  -> N.cond (foldr1 (N.bin N.And) patternChecks)
                   (N.let' patternBinds body')
                   (N.var "fail__")
...
unguard
  :: Either [(Guard Ann, Expr Ann)] (Expr Ann) -> N.Expr -> Convert N.Expr
...
unbinder (VarBinder _ name) scrut =
  pure $ (\name' -> ([], [(name', scrut)])) $ N.mkVar name
unbinder (ConstructorBinder (_, _, _, Just IsNewtype) _ _ [field]) scrut =
  unbinder field scrut
unbinder (ConstructorBinder ann _ (P.Qualified _ (P.ProperName tag)) fields) scrut
  = localAnn ann
    $   mappend ([N.bin N.Equals (N.sel scrut "tag__") (N.string tag)], [])
    .   mconcat
    <$> zipWithM (\binder field -> unbinder binder (N.sel scrut field))
                 fields
                 (N.numberedKeys "field__")
...
unbinder (LiteralBinder ann lit) scrut' = localAnn ann $ litBinder lit scrut'
 where
  litBinder
    :: Literal (Binder Ann) -> N.Expr -> Convert ([N.Expr], [(N.Var, N.Expr)])
  litBinder (NumericLiteral (Left n)) scrut =
    pure ([N.bin N.Equals scrut (N.int n)], [])
  litBinder (NumericLiteral (Right x)) scrut =
    pure ([N.bin N.Equals scrut (N.double x)], [])
  litBinder (StringLiteral str) scrut =
    (\str' -> ([N.bin N.Equals scrut (N.string str')], [])) <$> string str
  litBinder (CharLiteral char) scrut =
    pure ([N.bin N.Equals scrut (N.string (T.singleton char))], [])
  ...
  litBinder (ArrayLiteral as) scrut =
    mappend
        ( [ N.bin N.Equals
                  (N.app (N.var "length") scrut)
                  (N.int (fromIntegral n))
          ]
        , []
        )
      .   mconcat
      <$> zipWithM (\binder ix -> unbinder binder (N.index scrut ix))
                   as
                   [1 :: Integer ..]
    where n = length as
  litBinder (ObjectLiteral fields) scrut =
    mconcat
      <$> traverse
            (\(field, binder) ->
              unbinder binder (N.sel scrut (N.stringKey field))
            )
            fields

attrs :: [(PSString, Expr Ann)] -> Convert N.Expr
attrs = fmap N.assoc . traverse attr
  where attr (string, body) = (N.stringKey string, ) <$> expr body

string :: PSString -> Convert Text
string str =
  pure . T.pack . map (toEnum . fromIntegral) . toUTF16CodeUnits $ str
...
literal (BooleanLiteral b        ) = pure $ bool (N.var "F") (N.var "T") b
